...
# 1. The sub_0x37AE0 - SID 0xA3 handler
disasm(0x37AE0, 120, 'sub_37AE0 (SID 0xA3 handler?)')

# 2. The 4th lock write at 0x396DA
disasm(0x396B0, 80, 'sub containing lock write at 0x396DA')

# 3. Full SID dispatch chain 0x348E0-0x34AC0 with all SIDs
disasm(0x348E0, 300, 'Full SID dispatch 0x348E0-0x34AC0')

# 4. Search for ALL service bytes > 0x7F (manufacturer-specific) in dispatch code
print()
print('=== All manufacturer-specific SID bytes (>0x7F) compared in entire firmware ===')
sids_found = {}
for a in range(0x30000, 0x60000, 2):
    ...
    if (w>>8)==0x88:
        imm = w&0xFF
        if imm > 0x7F:
            sids_found[imm] = sids_found.get(imm,[])
            sids_found[imm].append(a)
for sid, addrs in sorted(sids_found.items()):
    print(f'  SID 0x{sid:02X}: {[hex(a) for a in addrs[:5]]}')

# 5. Check sub_37AE0 area for calls to sub_6EBD8 with reset values
print()
print('=== EEPROM write patterns in 0x37000-0x3A000 (around sub_37AE0) ===')
...
while True:
    ...
    if pos < 0: break
    lpool_addr = pos + BASE
    for a in range(max(0x37000, lpool_addr-0x1000), min(0x3A000, lpool_addr+4), 2):
        cw = fw_u16(a)
        if (cw>>12)==0xD:
            i8=cw&0xFF; nn=(cw>>8)&0xF
            t=(a&~3)+4+i8*4
            if t==lpool_addr:
                for j in range(a, a+20, 2):
                    jw = fw_u16(j)
                    if (jw&0xF00F)==0x400B and ((jw>>8)&0xF)==nn:
                        print(f'  sub_6EBD8 called at 0x{j:08X}')
                        for c in range(j-20, j+4, 2):
                            ins2,_ = sh2_one(c)
                            mark = ' <--' if c==j else ''
