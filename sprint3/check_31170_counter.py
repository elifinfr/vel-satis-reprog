...
# 1. sub_310BC complete (the function that actually handles the key check with EEPROM writes)
disasm(0x310BC, 160, 'sub_310BC complete (key verification + EEPROM writes)')

# 2. What's at 0x31578 area (another sub_6ED78 ref at 0x3147E)
disasm(0x31440, 80, 'sub around 0x3147E (sub_6ED78 write)')

# 3. Check the actual sub_31170 focusing on its write calls
disasm(0x31050, 120, 'sub_31170 full (from 0x31050 to see all writes)')

# 4. Check sub_27C80 from 0x27CCC - what happens at RequestSeed when EEPROM[0x1FFF]==0x00?
# The 0xAA write must happen in this path
disasm(0x27CE0, 80, 'sub_27C80 RequestSeed path (after EEPROM read, looking for 0xAA write)')
