...
# 1. The fail path at 0x27D88 (after key check failure in sub_27C80)
disasm(0x27D88, 80, 'sub_27C80 fail path (0x27D88 - after key fail)')

# 2. sub_27D98 (success/fail branch target from 0x27D84)
disasm(0x27D98, 60, 'sub_27C80 @ 0x27D98')

# 3. Look at sub_27FA0 - sub_27E76 tail that was identified in old analysis
disasm(0x27FA0, 60, 'sub_27E76 tail @ 0x27FA0 (counter write?)')

# 4. sub @ 0x28084 - another tail in sub_27E76
disasm(0x28084, 60, 'sub @ 0x28084 (tail of sub_27E76)')

# 5. sub_AB676 context around 0xAB9A4 (write 0xAA to EEPROM[0x1FFF])
disasm(0xAB960, 80, 'sub_AB676 @ 0xAB9A4 (write 0xAA to EEPROM[0x1FFF])')

# 6. Check what KWP service calls the lock function
# The lock is called from sub_34C74 (at 0x34CF8)
# Find callers of sub_34C74 by searching the pointer 0x00034C74 in the data sections too
print()
print('=== Pointer to 0x00034C74 anywhere in firmware ===')
target = struct.pack('>I', 0x00034C74)
off = 0
count = 0
while count < 20:
    ...
    if pos < 0: break
    addr = pos + BASE
    print(f'  0x{addr:08X}: contains 0x00034C74')
    off = pos + 1
    count += 1

# 7. What is the function that contains sub_309BA?
# Find STS.L PR start before 0x309B6
print()
for addr in range(0x309B6, 0x30800, -2):
    w = fw_u16(addr)
    if w == 0x4F22:  # STS.L PR,@-R15
        print(f'Function start before 0x309B6: 0x{addr:08X}')
        break
disasm(0x30900, 60, 'sub containing 0x309BA (calls sub_6EBD8 R5=0x55)')
