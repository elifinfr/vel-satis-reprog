...
def disasm(addr, n=300, lbl='', stop_at_rts=False):
    if lbl: print(f'\n=== {lbl} @ 0x{addr:08X} ===')
    ...
    for i in range(n):
        ...
        # Annotate pointers
        extra = ''
        if 'JSR' in ins or 'BSR' in ins or 'BRA' in ins or 'BT' in ins or 'BF' in ins:
            pass
        print(f'  0x{pc:08X}: {fw_u16(pc):04X}  {ins}{extra}')
        if last_rts and stop_at_rts: break
        if any(ins.startswith(x) for x in ['RTS','RTE']): last_rts=True
        pc=next_pc

# Key section: the area around 0xCAB60-0xCAC00 where SID dispatch happens
# Disasm from CAAB0 to CAC50 (the service dispatch core)
disasm(0xCA9E0, 300, 'KWP service dispatch core (sub_CA098)')
