...
# 1. sub_2851C - key comparison / verification function
disasm(0x2851C, 100, 'sub_2851C (key comparison/verification)')

# 2. sub_2848C - seed generation function
disasm(0x2848C, 80, 'sub_2848C (seed generation)')

# 3. Find XOR constant used in key verification
# Look for XOR #imm or XOR R,R patterns in sub_2851C area
print()
print('=== XOR instructions in 0x28400-0x28700 range ===')
for a in range(0x28400, 0x28700, 2):
    w = fw_u16(a)
    # XOR Rm,Rn (0x200A) or XOR #imm,R0 (0xCA)
    if (w>>4)==0x200 and (w&0xF)==0xA:  # XOR Rm,Rn
        ins,_ = sh2_one(a)
        print(f'  0x{a:08X}: {w:04X}  {ins}')
    if (w>>8)==0xCA:  # XOR #imm,R0
        ins,_ = sh2_one(a)
        print(f'  0x{a:08X}: {w:04X}  {ins}')

# 4. Look for constant 0xFFFFFFFF or 0xFFFF in key verify area (possible XOR mask)
print()
print('=== 0xFFFF or 0xFFFFFFFF constants in sub_2851C literal pool ===')
# scan 0x285xx-0x288xx for literal pool values of 0xFFFF or 0xFFFF
for a in range(0x28500, 0x28900, 2):
    v = fw_u16(a)
    v32 = fw_u32(a)
    if v == 0xFFFF:
        print(f'  [0x{a:08X}] = 0xFFFF  (potential NOT mask word)')
    if v32 == 0xFFFFFFFF:
        print(f'  [0x{a:08X}] = 0xFFFFFFFF  (potential XOR/NOT mask dword)')

# 5. Also check sub_27C80 to find where the seed and key are compared
# The seed bytes stored in EEPROM[0x1FF3-0x1FF8], key provided by tool
# Find the comparison logic in sub_2851C to determine the algorithm
print()
print('=== Key constant data in sub_2851C area (0x285xx-0x287xx) ===')
for a in range(0x28510, 0x28700, 4):
    v = fw_u32(a)
    if v not in [0xFFFFFFFF, 0x00000000] and 0x400 <= v <= 0x100400:
        print(f'  [0x{a:08X}] = 0x{v:08X}  <- code pointer')
    elif v == 0xFFFFFFFF:
        print(f'  [0x{a:08X}] = 0xFFFFFFFF')
