...
# 1. Find KWP service dispatch table
# KWP service handlers are usually in a table of {service_id, handler_ptr}
# sub_27C80 is the SID27 handler. Find a table containing 0x00027C80

print('=== Finding KWP dispatch table containing sub_27C80 (0x00027C80) ===')
target = struct.pack('>I', 0x00027C80)
off = 0
while True:
    pos = fw.find(target, off)
    if pos < 0: break
    addr = pos + BASE
    print(f'  0x{addr:08X}: contains 0x00027C80')
    # Show surrounding dwords (might be a table)
    for i in range(-4, 5):
        a = addr + i*4
        v = fw_u32(a)
...
# 2. Try to find service table by looking for patterns like
# byte SID followed by pointer to handler
# Common pattern in Denso: table of [1 byte service_id + 3 bytes pad + 4 bytes handler_ptr]
# OR [2 bytes + 2 bytes flags + 4 bytes ptr]
# Let me look at the areas near sub_27C80
print()
print('=== Looking at area around sub_27C80 for dispatch structures ===')
# Print 16 dwords before sub_27C80 address in tables
for addr in [0x27C80]:
    print(f'Context around 0x{addr:08X}:')
    for i in range(-8, 8):
        a = addr + i*4
        v = fw_u32(a)
        # Check if each dword looks like a code address in firmware range
        if 0x400 <= v <= 0x100400:
            print(f'    [0x{a:08X}] = 0x{v:08X}  <- code pointer')
        else:
            print(f'    [0x{a:08X}] = 0x{v:08X}')

# 3. Also find KWP handlers for other services by searching for typical NRC responses
# Look for services 0x23 (ReadMemoryByAddress), 0x3D (WriteMemoryByAddress), 0x14 (ClearDTC)
# These services usually start with reading the service byte and checking conditions
# Let's search for addresses of typical KWP handlers

# Search for TRAPA instructions used as service entry points?
# Actually look for the dispatch function that routes SIDs
# Sub_27C80 might be called from the main KWP dispatch
# Find what calls sub_27C80
print()
print('=== What calls sub_27C80? ===')
TARGET27C80 = 0x00027C80
off = 0
while True:
    pos = fw.find(struct.pack('>I', TARGET27C80), off)
    if pos < 0: break
    addr = pos + BASE
    # Check nearby for JSR
    for c in range(addr-8, addr+10, 2):
        cw = fw_u16(c)
        if (cw&0xF00F)==0x400B:  # JSR @Rn
            rn = (cw>>8)&0xF
            print(f'  JSR @R{rn} @ 0x{c:08X} (ptr loaded nearby 0x{addr:08X})')
    off = pos + 1

# 4. Look at the literal pool reference patterns around 0x27C80
# Show hex dump around the area where 0x27C80 pointer appears most
print()
print('=== Show raw dwords at locations referencing sub_27C80 ===')
off = 0
while True:
    pos = fw.find(struct.pack('>I', TARGET27C80), off)
    if pos < 0: break
    addr = pos + BASE
    # Show 8 dwords around this
    print(f'At 0x{addr:08X}:')
    for i in range(-2, 4):
        a = addr + i*4
        print(f'  [0x{a:08X}] = 0x{fw_u32(a):08X}')
    off = pos + 1
