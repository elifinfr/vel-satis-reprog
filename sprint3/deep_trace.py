...
def disasm(addr, n=150, lbl='', stop_at_rts=True):
    if lbl: print(f'\n=== {lbl} @ 0x{addr:08X} ===')
    pc=addr; last_rts=False
    for i in range(n):
        ins,next_pc=sh2_one(pc)
        print(f'  0x{pc:08X}: {fw_u16(pc):04X}  {ins}')
        if last_rts and stop_at_rts: break
        if any(ins.startswith(x) for x in ['RTS','RTE','JMP ','BRA ']): last_rts=True
        pc=next_pc

# 1. Continue sub_34C74 from the branch target at 0x34C92
disasm(0x34C92, 120, 'sub_34C74 second path (0x34C92)')

# 2. Check the "sub_34D10" label
disasm(0x34D10, 80, 'sub_34D10 (target of BRA from sub_34C74)')

# 3. Check call site 0x309BA (sub_6EBD8 with R5=0x55)
disasm(0x30970, 60, 'Zone 0x309BA (sub_6EBD8 call with R5=0x55)')

# 4. Find callers of sub_34C74 in broader range
print()
print('=== Callers of sub_34C74 (0x00034C74) - broader search 0x1000-0x60000 ===')
TARGET = 0x00034C74
for a in range(0x1000, 0x60000, 2):
    w = fw_u16(a)
    if (w>>12)==0xB:  # BSR
        disp=w&0xFFF
        if disp&0x800: disp|=0xFFFFF000
        ...
        if t == TARGET:
            print(f'  BSR @ 0x{a:08X} -> sub_34C74')
    elif (w>>12)==0xD:
        ...
        if v == TARGET:
            for c in [a+2,a+4,a+6]:
                ...
                if cw==(0x4000|(nn<<8)|0x0B):
                    print(f'  JSR @ 0x{c:08X} -> sub_34C74 (MOV.L @ 0x{a:08X})')

# 5. Scan sub_27C80 range for EEPROM write (0xAA marker)
# The RequestSeed path must write 0xAA after generating the seed
# sub_27C80 is the SID27 handler, check from sub_283EC to sub_2848C
print()
print('=== sub_283EC to sub_2848C: write 0xAA to EEPROM[0x1FFF] ===')
disasm(0x28466, 80, 'sub_283EC path after seed gate (0x28466)')

# 6. Show all calls to sub_6EBD8 and sub_6ED78 across entire firmware 0x1000-0xD0000
print()
print('=== ALL write EEPROM call sites (entire firmware) with context ===')
T6EBD8 = 0x0006EBD8
T6ED78 = 0x0006ED78
        ...
        if v in (T6EBD8, T6ED78):
            ...
            for c in [a+2,a+4,a+6]:
                ...
                if cw==(0x4000|(nn<<8)|0x0B):
                    ...
                    if (ds>>8)==0xE5:
                        r5v = ds&0xFF
                        if r5v&0x80: r5v|=0xFFFFFF00
                        r5_s = f' R5_delay=0x{r5v&0xFF:02X}'
                    # scan back for R4 load
                    r4_s = ''
                    for back in range(a-40, a+4, 2):
                        bw = fw_u16(back)
                        if (bw>>12)==0x9 and ((bw>>8)&0xF)==4:
                            bi8=bw&0xFF; bt=back+4+bi8*2
                            bv = fw_u16(bt)
                            r4_s = f' R4_w=0x{bv:04X}'
                        elif (bw>>12)==0xE and ((bw>>8)&0xF)==4:
                            bv = bw&0xFF
                            if bv&0x80: bv|=0xFFFFFF00
                            r4_s = f' R4_imm=0x{bv&0xFF:02X}'
                    print(f'  sub_{fname} @ 0x{c:08X}{r5_s}{r4_s}')
