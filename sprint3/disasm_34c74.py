...
def sh2_one(pc):
    ...
    if hi==0x5: return f'MOV.L @(0x{d*4:X},R{mm})',pc+2
    if hi==0x3:
...
# 1. Full function at 0x34C74 (contains the LOCK write at 0x34CF8)
disasm(0x34C74, 150, 'sub_34C74 (contains EEPROM[0x1FFF]=0x55 write)')

# 2. Find callers of sub_34C74
print()
print('=== Callers of sub_34C74 ===')
TARGET = 0x00034C74
for a in range(0x20000, 0x36000, 2):
    w = fw_u16(a)
    if (w>>12)==0xB:  # BSR
        disp=w&0xFFF
        if disp&0x800: disp|=0xFFFFF000
...
# 3. Scan for 0xAA write to any EEPROM func
print()
print('=== Scan: EEPROM write with value 0xAA (delay slot MOV #0xAA,R5) ===')
T6EBD8 = 0x0006EBD8
T6ED78 = 0x0006ED78
    ...
    if (w>>12)==0xD:
        ...
        if v in (T6EBD8, T6ED78):
            ...
            for c in [a+2,a+4,a+6]:
                cw=fw_u16(c)
                if cw==(0x4000|(nn<<8)|0x0B):
                    ds = fw_u16(c+2)  # delay slot
                    # Check R5 value (either in delay slot or just before)
                    r5_imm = None
                    if (ds>>8)==0xE5:
                        r5_imm = ds&0xFF
                        if r5_imm&0x80: r5_imm|=0xFFFFFF00
                    # Also check for R4 = 0x1FFF nearby
                    r4_is_1fff = False
                    for back in range(a-20, a+4, 2):
                        bw = fw_u16(back)
                        if (bw>>12)==0x9 and ((bw>>8)&0xF)==4:
                            bi8=bw&0xFF; bt=back+4+bi8*2
                            if fw_u16(bt)==0x1FFF:
                                r4_is_1fff = True
                    print(f'  {fname} @ 0x{c:08X}: delay_R5={hex(r5_imm) if r5_imm is not None else "?"}, R4_is_0x1FFF={r4_is_1fff}')
