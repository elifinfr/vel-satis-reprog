...
# -*- coding: utf-8 -*-
"""
Analyse finale : trouver le write EEPROM[0x1FFF] dans le chemin SID27
et identifier si KWP 0x3D ou autre service peut resetter le compteur.
"""
import struct, sys
...
# ---- 1. sub_6EBD8 (deuxieme driver EEPROM write) ----
disasm(0x6EBD8, 60, 'sub_6EBD8 (autre driver write EEPROM)')

# ---- 2. sub_31170 suite avec toutes les ecritures ----
disasm(0x311CE, 80, 'sub_31170 partie ecriture EEPROM (apres les lectures 0x1FBE/0x1FBF)')

# ---- 3. Chercher ALL references a sub_6EBD8 ----
print()
print('=== Recherche refs a sub_6EBD8 (0x0006EBD8) ===')
target = struct.pack('>I', 0x0006EBD8)
off=0
while True:
...
# ---- 4. Zone 0xAABBC (appelant de sub_AB676) ----
disasm(0xAAB90, 60, 'Zone 0xAABBC (appelant sub_AB676)')

# ---- 5. Chercher les writes EEPROM 0x1FFF/0x1FF0 dans sub_31170 ----
# sub_31170 commence a 0x31170 et finit quelque part autour 0x312C0
# Chercher les instructions write EEPROM dans cette region
print()
print('=== Dans sub_31170 (0x31170-0x312C0): instructions write EEPROM ===')
for a in range(0x31170, 0x312C0, 2):
    ins, _ = sh2_one(a)
    if ('JSR' in ins or 'BSR' in ins) and '0x0003' not in ins:
        v = fw_u32(a)
        print(f'  0x{a:08X}: {fw_u16(a):04X}  {ins}')
    # Chercher load de 0x1FFF ou 0x1FF0 dans R4
    w = fw_u16(a)
    if (w>>12) == 0x9:
        i8 = w & 0xFF
        t = a + 4 + i8*2
        val = fw_u16(t)
        if val in [0x1FFF, 0x1FF0, 0x1FFE, 0x1FF3, 0x1FF4, 0x1FF5, 0x1FF6, 0x1FF7, 0x1FF8, 0x1FBE, 0x1FBF]:
            print(f'  0x{a:08X}: {w:04X}  MOV.W @(PC),R{(w>>8)&0xF} = 0x{val:04X} (EEPROM idx)')

# ---- 6. Identifier service KWP utilisant sub_297A0 (autre ref 0x1FFF) ----
# 0x297A0 charge 0x1FFF, c est peut-etre le write du compteur pendant auth fail
disasm(0x29780, 80, 'sub @ 0x29780 (charge 0x1FFF - possible write counter)')
