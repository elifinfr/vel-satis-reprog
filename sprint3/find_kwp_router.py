...
# 1. KWP dispatcher with CMP/EQ #0x27 at 0x34176 - find function start
# Look for STS.L PR,@-R15 (function prologue) before 0x34176
print('Finding function start before 0x34176:')
for a in range(0x34176, 0x33800, -2):
    w = fw_u16(a)
    if w == 0x4F22:  # STS.L PR,@-R15
        print(f'  Function start: 0x{a:08X}')
        break

disasm(0x340E0, 160, 'KWP dispatcher area (CMP/EQ #0x27 at 0x34176)')

# 2. KWP dispatcher area with CMP/EQ #0x27 at 0x34A42
print('\nFinding function start before 0x34A42:')
for a in range(0x34A42, 0x34700, -2):
    w = fw_u16(a)
    if w == 0x4F22:
        print(f'  Function start: 0x{a:08X}')
        break

disasm(0x349C0, 120, 'KWP dispatcher area 2 (CMP/EQ #0x27 at 0x34A42)')

# 3. Around 0x34B14
disasm(0x34AE0, 80, 'KWP dispatcher area 3 (CMP/EQ #0x27 at 0x34B14)')

# 4. Also look for CMP/EQ with other SID values to find the full dispatch chain
print()
print('=== All CMP/EQ with KWP SID bytes in 0x33000-0x36000 range ===')
kwp_sids = {0x10,0x11,0x14,0x18,0x1A,0x1C,0x20,0x21,0x22,0x23,0x27,0x2C,0x2E,0x34,0x35,0x36,0x37,0x3D}
for a in range(0x33000, 0x36000, 2):
    w = fw_u16(a)
    if (w>>8) == 0x88:  # CMP/EQ #imm,R0
        imm = w & 0xFF
        if imm in kwp_sids:
            ins,_ = sh2_one(a)
            print(f'  0x{a:08X}: {w:04X}  {ins}')
    if (w>>8) == 0xE0:  # MOV #imm,R0 (might precede CMP)
        imm = w & 0xFF
        if imm in kwp_sids:
            ins,_ = sh2_one(a)
            print(f'  0x{a:08X}: {w:04X}  {ins}  [MOV]')
