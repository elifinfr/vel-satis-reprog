...
# 1. sub_6F3FE - the I2C address mapper (called by both read and write drivers with R4=EEPROM_index)
disasm(0x6F3FE, 100, 'sub_6F3FE (I2C address mapper, R4=EEPROM index)')

# 2. Also see sub_6EBD8 literal pool at 0x6ECB0 area for I2C slave addr
print()
print('=== Literal pool values in sub_6EBD8 (0x6ECA0-0x6ECE0) ===')
for a in range(0x6ECA0, 0x6ECE0, 2):
    v = fw_u16(a)
    print(f'  [0x{a:08X}] = 0x{v:04X}')

# 3. sub_6ED78 - the other EEPROM write function
disasm(0x6ED78, 80, 'sub_6ED78 (EEPROM multi-byte write?)')

# 4. SID 0x23 handler at 0x3B1B4 onwards
disasm(0x3B180, 120, 'SID 0x23 (ReadMemByAddr) handler context @ 0x3B1B0')

# 5. What I2C addresses does the EEPROM use?
# For AT24Cxx: Write = 0xA0|(page<<1), Read = 0xA1|(page<<1)
# Search for 0xA0 being loaded as an I2C address in sub_6F3FE area
print()
print('=== Looking for I2C device address construction in 0x6F300-0x6F500 ===')
for a in range(0x6F300, 0x6F500, 2):
    w = fw_u16(a)
    if w == 0xE0A0:  # MOV #0xA0,R0 (but A0 = -96 signed, so MOV #-96,R0 = E0A0)
        print(f'  0x{a:08X}: {w:04X}  MOV #0xA0(-96),R0')
    if (w>>8)==0xE0:
        imm = w&0xFF
        if imm >= 0xA0:
            ins,_ = sh2_one(a)
            print(f'  0x{a:08X}: {w:04X}  {ins}  (high byte imm)')
    # Also look for OR/AND #0xA0 patterns
    if w in [0xCBA0, 0xC9A0, 0xCAA0]:
        ins,_ = sh2_one(a)
        print(f'  0x{a:08X}: {w:04X}  {ins}  (possible I2C addr operation)')

# 6. Check the sub_6F3FE literal pool for constants
print()
print('=== Literal pool in sub_6F3FE area (0x6F480-0x6F540) ===')
for a in range(0x6F460, 0x6F540, 2):
    v = fw_u16(a)
    print(f'  [0x{a:08X}] = 0x{v:04X}')
