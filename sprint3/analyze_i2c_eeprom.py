...
# 1. sub_6EBD8 - the EEPROM write driver - understand I2C slave addr and addressing
disasm(0x6EBD8, 120, 'sub_6EBD8 (EEPROM write driver)')

# 2. sub_6F0E8 - EEPROM read driver
disasm(0x6F0E8, 80, 'sub_6F0E8 (EEPROM read driver)')

# 3. Look for I2C slave address constants (0xA0, 0xA2, 0xA4... for AT24Cxx)
print()
print('=== I2C slave address patterns in sub_6EBD8 area (0x6EB00-0x6EE00) ===')
for a in range(0x6EB00, 0x6EE00, 2):
    w = fw_u16(a)
    # MOV #0xA0,R0 or similar I2C addresses
    if (w>>8)==0xE0:
        imm = w&0xFF; uimm = imm if imm >= 0 else imm+256
        if uimm in [0xA0, 0xA2, 0xA4, 0xA6, 0xA8, 0xAA, 0xAE, 0x50, 0x51, 0x52, 0xA1, 0xA3]:
            ins,_ = sh2_one(a)
            print(f'  0x{a:08X}: {w:04X}  {ins}  (I2C slave addr candidate)')
    # Also look for byte values 0xA0/0xA1 in MOV.W @PC literals
    if (w>>12)==0x9:  # MOV.W @PC
        i8=w&0xFF; nn=(w>>8)&0xF; t=a+4+i8*2; v=fw_u16(t)
        if (v&0xFF) in [0xA0, 0xA1, 0xA2, 0xA3] or v in [0xA0, 0xA1, 0xA2, 0xA3]:
            ins,_ = sh2_one(a)
            print(f'  0x{a:08X}: MOV.W @PC -> 0x{v:04X}  (possible I2C addr)')

# 4. IIC register base: look for 0xF748 or 0xFFFFF748 in sub_6EBD8 area
print()
print('=== IIC register references in sub_6EBD8 area ===')
for a in range(0x6E800, 0x6F200, 2):
    w = fw_u16(a)
    if (w>>12)==0x9:  # MOV.W @PC
        i8=w&0xFF; nn=(w>>8)&0xF; t=a+4+i8*2; v=fw_u16(t)
        if 0xF700 <= v <= 0xF800:
            ins,_ = sh2_one(a)
            print(f'  0x{a:08X}: {w:04X}  {ins}  -> 0x{v:04X} (FFFF{v:04X} IO register)')
    if (w>>12)==0xD:  # MOV.L @PC
        i8=w&0xFF; nn=(w>>8)&0xF; t=(a&~3)+4+i8*4; v=fw_u32(t)
        if 0xFFFFF700 <= v <= 0xFFFFF800:
            ins,_ = sh2_one(a)
            print(f'  0x{a:08X}: {w:04X}  {ins}  -> 0x{v:08X} (IO register)')

# 5. Find and disassemble the SID 0x23 (ReadMemoryByAddress) handler
# From prior scan: CMP/EQ #0x23 at... let me find it
print()
print('=== CMP/EQ #0x23 (ReadMemByAddr) locations ===')
for a in range(0x30000, 0x60000, 2):
    w = fw_u16(a)
    if w == 0x8823:
        print(f'  0x{a:08X}: CMP/EQ #0x23,R0')
        # Show 20 bytes context
        for c in range(a-4, a+12, 2):
            ins2,_ = sh2_one(c)
            mark = ' <--' if c==a else ''
