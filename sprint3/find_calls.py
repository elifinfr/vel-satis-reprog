...
# Check what's at 0x3129C
print('=== Raw dword at 0x3129C (claimed sub_6EBD8 ref) ===')
print(f'  0x{0x3129C:08X}: 0x{fw_u32(0x3129C):08X}  (sub_6EBD8 = 0x0006EBD8)')

# Disasm code around it to see if it is a literal pool
disasm(0x31280, 40, 'Code around 0x3129C')

# Find actual JSR call sites to sub_6EBD8 in the auth region 0x27000-0x36000
print()
print('=== Actual JSR->sub_6EBD8 call sites in 0x27000-0x36000 ===')
TARGET6EBD8 = 0x0006EBD8
TARGET6ED78 = 0x0006ED78
for target_fn, name in [(TARGET6EBD8, 'sub_6EBD8'), (TARGET6ED78, 'sub_6ED78')]:
    print(f'\n  Searching for {name} = 0x{target_fn:08X}:')
    for base_a in range(0x27000, 0x36000, 2):
        w = fw_u16(base_a)
        if (w>>12) == 0xD:  # MOV.L @(PC+x),Rn
            nn = (w>>8)&0xF
            i8 = w&0xFF
            t = (base_a & ~3) + 4 + i8*4
            v = fw_u32(t)
            if v == target_fn:
                # Check if next few instructions include JSR @Rn
                for check in [base_a+2, base_a+4, base_a+6]:
                    cw = fw_u16(check)
                    if cw == (0x4000|(nn<<8)|0x0B):  # JSR @Rn
                        # What's in R4/R5 at this point?
                        # Look back to find the EEPROM index loaded into R4
                        r4_val = None
                        for back in range(base_a-40, base_a, 2):
                            bw = fw_u16(back)
                            if (bw>>12)==0x9 and ((bw>>8)&0xF)==4:  # MOV.W @PC,R4
                                bi8 = bw&0xFF
                                bt = back+4+bi8*2
                                r4_val = fw_u16(bt)
                            elif (bw>>12)==0xE and ((bw>>8)&0xF)==4:  # MOV #imm,R4
                                r4_val = bw&0xFF
                                if r4_val&0x80: r4_val |= 0xFFFFFF00
                        print(f'    0x{base_a:08X}: JSR->R{nn} @ 0x{check:08X}  R4_guess=0x{r4_val:04X}' if r4_val else f'    0x{base_a:08X}: JSR->R{nn} @ 0x{check:08X}')
                        break

# Also check what's written during the SendKey fail path
# sub_27E76 is the SendKey handler - trace the write after key comparison failure
print()
print('=== sub_27E76 SendKey fail path: looking for EEPROM write ===')
disasm(0x27E76, 100, 'sub_27E76 SendKey handler')
