...
# 1. Instruction qui charge 0x1FFF depuis 0xABA7C
print('=== Instructions MOV.W depuis 0xABA7C (=0x1FFF) ===')
for a in range(0xAB676, 0xABA7C, 2):
    w = fw_u16(a)
    if (w>>12) == 0x9:
        i8 = w & 0xFF
        t = a + 4 + i8*2
        if t == 0xABA7C:
            print(f'  Trouvee @ 0x{a:08X}: {w:04X}  MOV.W @(PC+{i8*2}),R{(w>>8)&0xF}  <- 0x1FFF')

# 2. sub_AB676
disasm(0xAB676, 80, 'sub_AB676 debut')

# 3. Zone de l appelant 0xAAABBC
disasm(0xAAAB70, 60, 'Appelant sub_AB678 via 0xAAABBC')

# 4. Remonter encore plus haut pour trouver le service KWP
# Chercher les appels vers 0xAA000-0xAAC00 depuis 0x20000-0xA0000
print()
print('=== Cross-refs vers 0xAA000-0xAAE00 depuis zone KWP 0x20000-0xA0000 ===')
found = []
for a in range(0x20000, 0xA0000, 2):
    w = fw_u16(a)
    # BSR relative
    if (w>>12) == 0xB:
        disp = w&0xFFF
        ...
        if 0xAA000 <= t <= 0xAAE00:
            found.append((a, t, f'BSR 0x{t:08X}'))
    # BRA - probably not for inter-function calls, skip
for a,t,ins in found[:20]:
    print(f'  @ 0x{a:08X}: {ins}')
