...
def sh2_one(pc):
    w=fw_u16(pc); hi=(w>>12)&0xF; nn=(w>>8)&0xF; mm=(w>>4)&0xF; d=w&0xF; imm8=w&0xFF
    if hi==0xA:
        disp=w&0xFFF
    ...
    if hi==0xD: t=(pc&~3)+4+imm8*4; v=fw_u32(t); return f'MOV.L @(PC+{imm8*4}),R{nn} ;[0x{t:08X}]=0x{v:08X}',pc+2
    if hi==0x9: t=pc+4+imm8*2; v=fw_u16(t); return f'MOV.W @(PC+{imm8*2}),R{nn} ;[0x{t:08X}]=0x{v:04X}',pc+2
    if hi==0xE: imm=imm8; imm=imm|0xFFFFFF00 if imm&0x80 else imm; return f'MOV  #{imm},R{nn}',pc+2
    if hi==0x7: imm=imm8; imm=imm|0xFFFFFF00 if imm&0x80 else imm; return f'ADD  #{imm},R{nn}',pc+2
    if hi==0x2:
    ...
    if hi==0x0:
        ...
        op=(mm<<4)|d
        t0={0x02:f'STC SR,R{nn}',0x03:f'BSRF R{mm}',0x0A:f'STS MACH,R{nn}',0x12:f'STC GBR,R{nn}',0x1A:f'STS MACL,R{nn}',0x22:f'STC VBR,R{nn}',0x29:f'MOVT R{nn}',0x2A:f'STS PR,R{nn}',0x3B:f'BRAF R{mm}',0x04:f'MOV.B R{mm},@(R0,R{nn})',0x05:f'MOV.W R{mm},@(R0,R{nn})',0x06:f'MOV.L R{mm},@(R0,R{nn})',0x07:f'MUL.L R{mm},R{nn}',0x0C:f'MOV.B @(R0,R{mm}),R{nn}',0x0D:f'MOV.W @(R0,R{mm}),R{nn}',0x0E:f'MOV.L @(R0,R{mm}),R{nn}',0x0F:f'MAC.L @R{mm}+,@R{nn}+'}
    ...
    if (w>>8)==0x88: imm=imm8; imm=imm|0xFFFFFF00 if imm&0x80 else imm; return f'CMP/EQ #{imm&0xFF},R0',pc+2
...
# 1. Trouver le debut de la fonction autour 0xABAA0 (write EEPROM[0x1FFF])
print('=== Recherche debut fonction avant 0xABAA0 ===')
for addr in range(0xABAA0, 0xAB000, -2):
    w = fw_u16(addr)
    if w == 0x4F22:  # STS.L PR,@-R15
        print(f'  Possible debut: 0x{addr:08X}')
        break

disasm(0xAB9E0, 100, 'Fonction ~0xAB9E0 (write EEPROM[0x1FFF] = ???)')

# 2. Trouver ce qui est ecrit dans EEPROM[0x1FFF] - regarder l instruction avant l appel
print()
print('=== Contexte exact de l appel write @ 0xABAA0 ===')
# L appel sub_6ED78 est a 0xABAA0 (le pointeur 0x0006ED78 est stocke quelque part)
# Chercher le JSR ou BSR avant
disasm(0xABA60, 60, 'Zone avant write EEPROM[0x1FFF] @ 0xABA60')

# 3. Write EEPROM[0x1FF0] @ 0x3179C
disasm(0x31730, 80, 'Zone write EEPROM[0x1FF0] @ 0x3179C')

# 4. Chercher les services KWP accessibles depuis la zone 0xAB000
# Quels services appellent les fonctions dans cette region?
print()
print('=== Cross-refs vers 0xAB000-0xABC00 ===')
# Chercher les BSR/BRA vers cette region
for addr in range(0x1000, 0xAB000, 2):
    ins, _ = sh2_one(addr)
    if ('BSR' in ins or 'BRA' in ins or 'JSR' in ins):
        # Extraire l adresse cible
        try:
            target_str = ins.split('0x')[-1]
            target = int(target_str[:8], 16)
            if 0xAB000 <= target <= 0xABC00:
