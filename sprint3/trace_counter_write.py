...
def sh2_one(pc):
    ...
    if hi==0x0:
        ...
        # Check d (lower nibble) for Rm-based instructions
...
# 1. Continue sub_27E76 from where it left off (0x27F3C onwards)
disasm(0x27F3C, 120, 'sub_27E76 continuation (key compare + counter write)')

# 2. Check 0x309B6 - sub_6EBD8 call site
disasm(0x30970, 60, 'sub at 0x309B6 (calls sub_6EBD8)')

# 3. Check sub at 0x310F8 (calls sub_6EBD8 with R4=-3?)
disasm(0x310C0, 60, 'sub at 0x310F8 (calls sub_6EBD8)')

# 4. Scan for any place that loads 0x1FFF into R4/R5 then calls sub_6ED78 or sub_6EBD8
print()
print('=== Scan: load 0x1FFF into Rn then EEPROM write call ===')
for a in range(0x27000, 0x36000, 2):
    ...
    if (w>>12) == 0x9:
        ...
        if val == 0x1FFF:
            rn = (w>>8)&0xF
            print(f'  0x{a:08X}: MOV.W @PC,R{rn} = 0x1FFF  (EEPROM addr)')
