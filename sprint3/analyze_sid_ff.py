...
# 1. Full SID dispatch at 0x35218 (BRA target)
disasm(0x35218, 300, 'Full SID dispatch at 0x35218')

# 2. Context around CMP/EQ #0xFF at 0x33346
disasm(0x33300, 80, 'Context around CMP/EQ #0xFF at 0x33346')

# 3. Context around CMP/EQ #0xFF at 0x372EC
disasm(0x372C0, 80, 'Context around CMP/EQ #0xFF at 0x372EC')

# 4. What's at the handler for SID 0xFF?
# From the scan: 0x372EC, 0x37396, 0x374BA - all in the same function area
disasm(0x37260, 200, 'sub containing CMP/EQ #0xFF at 0x372EC')

# 5. Check if any EEPROM[0x1FFF]=0x00 or reset write exists in the 0x37xxx SID 0xFF handler area
print()
print('=== All EEPROM sub_6EBD8 calls in 0x37000-0x38000 ===')
...
while True:
    ...
    if pos < 0: break
    lpool_addr = pos + BASE
    if 0x37000 <= lpool_addr < 0x38000:
        print(f'  sub_6EBD8 pointer at literal pool 0x{lpool_addr:08X}')
        # Find MOV.L @PC loading this
        for a in range(max(0x37000,lpool_addr-200), min(0x38000, lpool_addr), 2):
            cw = fw_u16(a)
            if (cw>>12)==0xD:
                i8=cw&0xFF; nn=(cw>>8)&0xF
                t=(a&~3)+4+i8*4
                if t==lpool_addr:
                    print(f'    Loaded at 0x{a:08X} into R{nn}')
                    for b in range(a, min(a+20, 0x38000), 2):
                        bw = fw_u16(b)
                        if (bw&0xF00F)==0x400B and ((bw>>8)&0xF)==nn:
                            print(f'    JSR @R{nn} at 0x{b:08X}')
                            # Show R4,R5 context
                            for c in range(b-20, b+4, 2):
                                ins2,_ = sh2_one(c)
                                mark = ' <--' if c==b else ''
