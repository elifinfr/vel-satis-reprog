...
def disasm(addr, n=200, lbl='', stop_on_rts=True):
    if lbl: print(f'\n=== {lbl} @ 0x{addr:08X} ===')
    pc=addr; last_rts=False
    for i in range(n):
        ins,next_pc=sh2_one(pc)
        print(f'  0x{pc:08X}: {fw_u16(pc):04X}  {ins}')
        if last_rts and stop_on_rts: break
        if any(ins.startswith(x) for x in ['RTS','RTE','JMP ','BRA ']): last_rts=True
        pc=next_pc

# 1. Main KWP dispatcher from function start at 0x34070
disasm(0x34070, 400, 'main KWP dispatcher (sub_34070)', stop_on_rts=False)

# 2. The area at 0x34940 that leads to sub_34C74
disasm(0x348E0, 80, 'area before 0x34940 (CMP/EQ #8 path to sub_34C74)')

# 3. Context of the "0x23 ReadMemoryByAddress" handler if found
# Let's find it: look for CMP/EQ #0x23
print()
print('=== CMP/EQ #0x23 (ReadMemoryByAddress) in 0x33000-0x36000 ===')
