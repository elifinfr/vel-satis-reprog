...
def sh2_one(pc):
    ...
    if hi==0x4:
        oc=(mm<<4)|d
        t4={0x00:f'SHLL R{nn}',0x00:f'SHLL R{nn}',0x01:f'SHLR R{nn}',0x02:f'STS.L MACH,@-R{nn}',0x04:f'ROTL R{nn}',0x05:f'ROTR R{nn}',0x06:f'LDS.L @R{nn}+,MACH',0x08:f'SHLL2 R{nn}',0x09:f'SHLR2 R{nn}',0x0A:f'LDS R{nn},MACH',0x0B:f'JSR  @R{nn}',0x0E:f'LDC R{nn},SR',0x10:f'DT   R{nn}',0x11:f'CMP/PZ R{nn}',0x15:f'CMP/PL R{nn}',0x16:f'LDS.L @R{nn}+,MACL',0x17:f'LDC.L @R{nn}+,GBR',0x18:f'SHLL8 R{nn}',0x19:f'SHLR8 R{nn}',0x1A:f'LDS R{nn},MACL',0x1B:f'TAS.B @R{nn}',0x1E:f'LDC R{nn},GBR',0x20:f'SHAL R{nn}',0x21:f'SHAR R{nn}',0x22:f'STS.L PR,@-R{nn}',0x23:f'STC.L VBR,@-R{nn}',0x24:f'ROTCL R{nn}',0x25:f'ROTCR R{nn}',0x26:f'LDS.L @R{nn}+,PR',0x27:f'LDC.L @R{nn}+,VBR',0x28:f'SHLL16 R{nn}',0x29:f'SHLR16 R{nn}',0x2A:f'LDS R{nn},PR',0x2B:f'JMP  @R{nn}',0x2E:f'LDC R{nn},VBR',0x13:f'STC.L GBR,@-R{nn}'}
...
# The pointer 0x00027C80 is at 0xCAC3C - this is a literal pool entry.
# Find the MOV.L @PC instructions that load from 0xCAC3C
# These would be at addresses near 0xCAC3C: MOV.L @(PC+offset),Rn
# where (pc&~3)+4+i8*4 == 0xCAC3C

# Scan code in area around 0xCA000-0xCB000 for MOV.L @PC loading from 0xCAC3C
print('=== MOV.L @PC instructions loading from 0xCAC3C ===')
for a in range(0xC9000, 0xCB000, 2):
    w = fw_u16(a)
    if (w>>12)==0xD:  # MOV.L @(PC+off),Rn
        i8 = w&0xFF; nn=(w>>8)&0xF
        t = (a&~3)+4+i8*4
        if t == 0xCAC3C:
            print(f'  0x{a:08X}: {w:04X}  MOV.L @(PC+{i8*4}),R{nn} -> loads 0x00027C80')

# Also check for JSR @Rn near where the value is loaded
# Find ALL JSR @Rn in the 0xCA000-0xCB000 range
print()
print('=== JSR @Rn in 0xCA000-0xCB000 range ===')
for a in range(0xCA000, 0xCB000, 2):
    w = fw_u16(a)
    if (w&0xF00F)==0x400B:  # JSR @Rn
        rn=(w>>8)&0xF
        ins,_ = sh2_one(a)
        print(f'  0x{a:08X}: {w:04X}  {ins}')

# Also find callers by looking at area just before 0xCAC3C (literal pool)
# Go back to find function boundary (STS.L PR,@-R15)
print()
print('=== Function start before 0xCAC3C ===')
for a in range(0xCAC3C, 0xCA000, -2):
    w = fw_u16(a)
    if w == 0x4F22:  # STS.L PR,@-R15
        print(f'  Function start: 0x{a:08X}')
        break

disasm(0xCAB00, 100, 'code near literal pool at 0xCAC3C')

# Also search ALL MOV.L @PC loading from 0xCAC3C across the whole firmware
print()
print('=== ALL MOV.L @PC loading from 0xCAC3C (entire firmware) ===')
for a in range(0x400, 0x100400, 2):
    w = fw_u16(a)
    if (w>>12)==0xD:  # MOV.L @(PC+off),Rn
        i8 = w&0xFF; nn=(w>>8)&0xF
        t = (a&~3)+4+i8*4
        if t == 0xCAC3C:
            print(f'  0x{a:08X}: {w:04X}  MOV.L @(PC+{i8*4}),R{nn} -> loads 0x00027C80')
            # show context
            for c in range(a-6, a+10, 2):
                ins2,_ = sh2_one(c)
                mark = ' <--' if c==a else ''
                print(f'    0x{c:08X}: {fw_u16(c):04X}  {ins2}{mark}')
            print()
