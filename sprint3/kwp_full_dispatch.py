...
# Full dispatch chain starting at 0x34940 (just before the SID comparisons at 0x349FC)
disasm(0x34940, 260, 'Full KWP dispatch chain (0x34940-0x34B80)')

# Also disasm the function from 0x34070 (start of function containing CMP/EQ #0x27 at 0x34176)
disasm(0x34070, 200, 'KWP dispatcher function @ 0x34070')

# Check entire firmware for CMP/EQ #0x3D
print()
print('=== CMP/EQ #0x3D anywhere in firmware ===')
count = 0
for a in range(0x400, 0x100400, 2):
    w = fw_u16(a)
    if w == 0x883D:  # CMP/EQ #0x3D,R0
        print(f'  0x{a:08X}: {w:04X}  CMP/EQ #0x3D,R0')
        count += 1
if count == 0:
    print('  (none found)')

# Also check for MOV #0x3D used as service byte
print()
print('=== MOV #0x3D (61) patterns in KWP range 0x30000-0x60000 ===')
count = 0
for a in range(0x30000, 0x60000, 2):
    w = fw_u16(a)
    if w == 0xE03D:  # MOV #0x3D,R0
        ins,_ = sh2_one(a)
